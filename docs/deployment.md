...
| `CORS_ALLOWED_ORIGINS` | no | chart (`django.corsAllowedOrigins`) | Normally empty: nginx proxies Django's paths, so the browser sees one origin. Setting it grants cross-origin *reads* to an unauthenticated client and nothing more — the session cookie is not sent cross-origin, so it does not make a frontend on a second hostname work; writes are the row below |
| `CSRF_TRUSTED_ORIGINS` | no | chart (`django.csrfTrustedOrigins`) | Normally empty, and a different question from the row above: that one is about *reading*, this one about *writing*. Django checks the origin a browser claims against the one it works out for itself, and this list is what admits a request when those disagree. The frontend forwards the browser's own host, so they agree and nothing here needs it — fill it in when an ingress rewrites the host, and the symptom that sends you here is every write refused with `CSRF Failed: Origin checking failed` |

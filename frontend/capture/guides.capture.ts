...
import { IMAGE_DIR, imagePath, REPO_ROOT, SHOTS } from "./shots";
...
test("every picture in the guides", async ({ page }) => {
  ...
  mkdirSync(`${REPO_ROOT}${IMAGE_DIR}`, { recursive: true });
  ...
  const whoAreYou = page.getByLabel(WHO_ARE_YOU);
  ...
  const kind = page.getByLabel(WHAT_IS_HAPPENING);
  ...
  await shoot(
    page,
    "volunteer-outbox",
    await around(outbox, page.getByRole("button", { name: SEND_NOW })),
  );

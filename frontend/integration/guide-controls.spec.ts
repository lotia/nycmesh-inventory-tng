...
import { DESK, PHONE, saveButton, scan, WALKING_THE_SCENE_MAY_TAKE } from "../capture/drive";
import {
  dressTheScene,
  ITEM_CODE,
  MEASURED_CODE,
...
test("the app still offers every control the guides name", async ({ page }) => {
  ...
  await page.getByLabel("What is happening").click();
  ...
  const missing = GUIDES.flatMap((guide) => {
    const reachable = SCREENS[guide].flatMap((screen) => [...offered[screen]]);
    return controlsIn(guide)
      .filter((control) => !reachable.includes(control))

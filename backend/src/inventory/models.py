...
class ItemIdentifier(models.Model):
    ...
    # PROTECT, and it is the whole of inventory-tng-6kyb. This was CASCADE, so
    # deleting an item took every string that had ever meant it -- a barcode
    # printed on the thing itself, a legacy NYCM code somebody still types --
    # without warning and without asking, exactly as `Label` did before 6pr.
    #
    # LOSING THE STRING IS NOT THE WORST OF IT. `value_normalised` is unique
    # across the whole table, which is what makes a scan resolve to exactly one
    # item; a deleted row frees that string, and it can then be created against
    # a DIFFERENT item. The barcode on the shelf goes on scanning, and answers
    # with something else.
    #
    # Not narrowed to the printed kinds. An `alias` is freed by the same delete
    # and re-attachable in the same way, so narrowing would leave the half that
    # answers wrongly and close only the half that answers not at all -- and
    # `on_delete` is per key rather than per row, so a narrower rule would be
    # application logic beside a database posture, which is the arrangement 6pr
    # rejected on `Label`.
    #
    # The consequence for a catalogue somebody is tidying is deliberate, and
    # docs/data-model.md is where it is spelled out.
    item = models.ForeignKey(Item, on_delete=models.PROTECT, related_name="identifiers")
    kind = models.CharField(max_length=20, choices=Kind)
    value = models.CharField(max_length=200)
    # `inventory.identifiers` is what this folds and why Python's answer is not
    # allowed to be it.
    #
    # WIDER THAN `value`, WHICH IS NOT A MISTAKE and is the one thing about
    # this column that is not in that module. NFC can lengthen a string --
    # worst case three characters out of one -- so 200 characters somebody may
    # legitimately type can be 600 here. Matching the two lengths would refuse
    # them with a `DataError`, which is not an `IntegrityError` and so is not
    # something `_identifiers._hold` can report as a conflict.
...
class VendorOffer(models.Model):
    ...
    # PROTECT, the other half of inventory-tng-6kyb, and the promise it keeps
    # is the docstring above: a historical purchase price stays recoverable.
    # This was CASCADE, so one delete of an unlabelled, unmoved item took the
    # whole series -- and unlike a sticker there is no physical artefact left
    # to notice the loss by. `vendor` was already PROTECT for the same reason
    # read the other way about, so the two keys of this row now agree.
    item = models.ForeignKey(Item, on_delete=models.PROTECT, related_name="offers")
